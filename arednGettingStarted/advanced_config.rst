...
Wireguard Tunneling Protocol
  ...
  .. note about time synchronisation:: WireGuard will not establish tunnels with unsynced clock from either the clients or the servers. Ensure that all nodes configured to use WireGuard have their NTP server reachable at boot. It is recommended to use the default ``pool.ntp.org`` value. If you have mesh based NTP server, you can advertise them as services and ensure offline time synchronisation of your whole mesh if Internet is not available.
